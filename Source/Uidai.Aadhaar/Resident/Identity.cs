...
namespace Uidai.Aadhaar.Resident
{
    ...
    public class Identity : IUsed, IXml
    {
        ...
        public void FromXml(XElement element)
        {
            ...
            if (value != null)
            {
                // Workaround for e-KYC response where dob is sent in dd-MM-yyyy format instead of yyyy-MM-dd.
                var dob = DateTimeOffset.MinValue;
                if (DateTimeOffset.TryParseExact(value, "dd-MM-yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out dob))
                    DateOfBirth = dob;
                else
                {
